...
/*
function atualizarStatusComIndice(indiceTarefa, statusParaAtualizar) {
    listaTarefas[indiceTarefa].atualizaStatus(statusParaAtualizar);
}
*/
    ...
    rl.question('Digite o número referente à opção desejada: ', (opcao) => {
        ...
        switch (opcao) {
            ...
            /*
                ...
                console.log("");
                console.log('antes da entrada')
                rl.question('Digite o número da tarefa que deseja atualizar o status: ', (numero) => {
                    indiceTarefaAtualizar = parseInt(numero) - 1; // Alterando o tipo da resposta do usuário para Int e adequando às posições do vetor listaTarefas
                    console.log('depois da entrada')
                    let flagMenuVisualizar = 1;
                    while (flagMenuVisualizar === 1) {
                        if (indiceTarefaAtualizar >= 0 && indiceTarefaAtualizar < listaTarefas.length) {
                            console.log("");
                            console.log("[1] - A fazer.");
                            console.log("[2] - Em andamento.");
                            console.log("[3] - Concluída.");
                            ...
                            rl.question('Digite o número do status que deseja adicionar à tarefa: ', (numStatus) => {
                                flagMenuVisualizar = 0;
                                let flagEntrada = 1; // Variável para detecção de entrada errada
                                while (flagEntrada === 1) {
                                    switch (numStatus) {
                                        case '1':
                                            statusParaAtualizar = 'A Fazer';
                                            flagEntrada = 0;
                                            break;
                                        case '2':
                                            statusParaAtualizar = 'Em Andamento';
                                            flagEntrada = 0;
                                            break;
                                        case '3':
                                            statusParaAtualizar = 'Concluída';
                                            flagEntrada = 0;
                                            break;
                                        default:
                                            console.log("Opção Inválida, selecione a tarefa novamente!");
                            ...
                            console.log('Número de tarefa Inválido, tente novamente!');
                            console.log("");
                            flagMenuVisualizar = 1;
                        }
                    }
                });

                indiceTarefaAtualizar = 0;
                statusParaAtualizar = 'Concluída';
                atualizarStatusComIndice(indiceTarefaAtualizar, statusParaAtualizar);
